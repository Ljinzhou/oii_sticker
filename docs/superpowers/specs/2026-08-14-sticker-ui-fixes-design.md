...
2. 便签交互模式**保留**顶部蒙版关闭按钮（✕，四按钮 ▽✎⚙✕）；**编辑模式**底部工具条**移除**关闭按钮（仅保存/取消/设置）。【用户澄清修正：此前方案理解反了】
...
- interact 蒙版保留 ✕ 按钮（四按钮 ▽ / ✎ / ⚙ / ✕）。—— 需求 2
- `onClosed`（:141-144）改为 `await invoke("hide_sticker_cmd", { id: stickerId })`（隐藏不销毁，配合 3.2 的同步）。

**`src/components/sticker/StickerEditor.vue`**
- 移除编辑工具条 ✕ 按钮（保存/取消/⚙ 保留）、`closed` emit，及 StickerWindow 中对 StickerEditor 的 `@closed` 绑定。
...
- 便签窗口「关闭」从**销毁**变为**隐藏**：数据保留、可经主控台「显示」恢复；与「关闭不删数据」既有约定一致。关闭入口：交互蒙版 ✕ 与主控台「隐藏」按钮。
...
- 新增 `StickerWindow.test.ts`：interact 蒙版四按钮（含 ✕）；点击蒙版 ✕ 调 `hide_sticker_cmd`。
- 新增 `StickerEditor.test.ts`：编辑工具条不含 ✕（仅保存/取消/设置）。
...
1. 主控台卡片无 "edit" 徽章；2. interact 蒙版为 ▽✎⚙✕ 四按钮、编辑工具条无 ✕；3. 关闭便签 → 主控台立即变「显示」→ 点击「显示」窗口恢复且 Rust 日志有 `[cmd] wake_sticker` 记录；4. `pnpm test`、`cargo test`、`cargo clippy` 全绿；5. 中文 git 提交。
